...
namespace cg::renderer
{
	...
	inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset)
	{
		...
		while (vertex_id < vertex_offset + num_vertexes) {
            ...
            int2 vertex_a = int2(vertices[0].x, vertices[0].y);
            int2 vertex_b = int2(vertices[1].x, vertices[1].y);
            int2 vertex_c = int2(vertices[2].x, vertices[2].y);

			auto edge = static_cast<float>(edge_function(vertex_a, vertex_b, vertex_c));

            const int2 min_boarder = int2{0, 0};
            const int2 max_boarder = int2{static_cast<int>(width - 1), static_cast<int>(height - 1)};
                ...
                for (int y = bb_begin.y; y <= bb_end.y; ++y) {
					...
					if (edge0 >= 0 && edge1 >= 0 && edge2 >= 0) {
						...
						if (depth_test(depth, static_cast<size_t>(x), static_cast<size_t>(y))) {
                            ...
                            render_target->item(static_cast<size_t>(x),
