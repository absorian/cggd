...
void cg::world::camera::set_theta(float in_theta) // deg
{
	theta = in_theta * static_cast<float>(M_PI) / 180;
}

void cg::world::camera::set_phi(float in_phi) // deg
{
    phi = in_phi * static_cast<float>(M_PI) / 180;
}

void cg::world::camera::set_angle_of_view(float in_aov) // deg
{
...
{
    float f = 1 / std::tanf(angle_of_view * 0.5f);
	return float4x4{
